...
SaveEventsDB::SaveEventsDB(sql::Connection* a_dbConnection, std::string const& a_tableName)
: m_dbConnection(a_dbConnection)
, m_tableName(a_tableName)
{
}

void SaveEventsDB::Save(std::vector<std::pair<std::string, std::string>> const& a_data)
{
    std::string columns = {"("};
    std::string values = {"("};
